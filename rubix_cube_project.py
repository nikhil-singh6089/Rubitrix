import cv2
import numpy as np
import kociemba.solver as Cube
import time
import colorama
...
print(rf"{GREEN}         | |____   |  |___|  |  | |_____| |  | |_______                                  |___|___|___|            ")
...
color = {#BGR VALUES (BLUE, GREEN, RED)
        'red'    : (0,0,255),
        'orange' : (0,165,255),
        'blue'   : (255,0,0),
        'green'  : (0,255,0),
...
def color_detect(h,s,v):
    # print(h,s,v)
    if ((h>=119 and h<=179) and (s>=147 and s<=255) and (v>=0 and v<=255)):
        return 'red'
    elif((h>=0 and h<=23) and (s>=70 and s<=255) and (v>=143 and v<=255)):
        return 'orange'
    elif((h>=14 and h<=38) and (s>=106 and s<=250) and (v>=101 and v<=191)):
        return 'yellow'
    elif((h>=41 and h<=77) and (s>=98 and s<=225) and (v>=0 and v<=255)):
        return 'green'
    elif((h>=97 and h<=158) and (s>=120 and s<=255) and (v>=67 and v<=255)):
        return 'blue'
    elif((h>=79 and h<=142) and (s>=0 and s<=101) and (v>=92 and v<=158)):
        return 'white'
    return 'white'
...
def process(operation):
    replace={
                ...
                "L1":[rotate,'left'],
                "L2":[rotate,'left','left'],
                "L3":[rotate,'left','left','left'],
                "L'":[revrotate,'left'],

                "R1":[rotate,'right'],
                "R2":[rotate,'right','right'],
                "R3":[rotate,'right','right','right'],
                "R'":[revrotate,'right'],

                "D1":[rotate,'down'],
                "D2":[rotate,'down','down'],
                "D3":[rotate,'down','down','down'],
                "D'":[revrotate,'down'],

                "B1":[rotate,'back'],
                "B2":[rotate,'back','back'],
                "B3":[rotate,'back','back','back'],
                "B'":[revrotate,'back']           
    } 
    for i in operation:
        for j in range(len(replace[i])-1):
            replace[i][0](replace[i][j+1])
        cv2.putText(preview, i, (700,50), font,1,(0,255,0), 1, cv2.LINE_AA)  
